...
class AccountController @Inject() (
  ...
  def createOrTransferLisaAccount(lisaManager: String): Action[AnyContent] =
    (validateHeader(parse) andThen validateLMRN(lisaManager)).async { implicit request =>
      ...
      logger.info(s"[AccountController][createOrTransferLisaAccount]  started lisaManager : $lisaManager")
      withValidJson[CreateLisaAccountRequest](
        {
          case createRequest: CreateLisaAccountCreationRequest   =>
            if (hasAccountTransferData(request.body.asJson.get.as[JsObject])) {
              auditService.audit(
                auditType = "transferAccount",
                path = getCreateOrTransferEndpointUrl(lisaManager),
                auditData = Map(ZREF -> lisaManager, "reasonNotCreated" -> "FORBIDDEN")
              )
              ...
              logger.info(s"[AccountController][createOrTransferLisaAccount]  in else block")
        ...
        Some { errors =>
          logger.warn(
            s"[AccountController][createOrTransferLisaAccount] Lisa Manager : $lisaManager The errors are " + errorConverter
              .convert(errors)
          )
  ...
  private def processAccountCreation(lisaManager: String, creationRequest: CreateLisaAccountCreationRequest)(implicit
    hc: HeaderCarrier,
    startTime: Long
  ) = {
    val action = "Created"
    ...
    hasValidDatesForCreation(lisaManager, creationRequest) { () =>
      service.createAccount(lisaManager, creationRequest).map {
        case CreateLisaAccountSuccessResponse(accountId) =>
          ...
          logger.info(
            s"[AccountController][processAccountCreation]  CreateLisaAccountSuccessResponse accountId : $accountId, lisaManager : $lisaManager"
          )
        ...
        case CreateLisaAccountAlreadyExistsResponse      =>
          logger.info(
            s"[AccountController][processAccountCreation] CreateLisaAccountAlreadyExistsResponse lisaManager : $lisaManager"
          )
          handleCreateOrTransferFailure(
            lisaManager,
            creationRequest,
            ErrorAccountAlreadyExists(creationRequest.accountId),
            action
          )
        case error: CreateLisaAccountResponse            =>
          val errorResponse = createLisaAccountErrorMap.getOrElse(error, ErrorInternalServerError)
          logger.error(
            s"[AccountController][processAccountCreation] CreateLisaAccountResponse lisaManager : $lisaManager, errorResponse: $errorResponse"
          )
          handleCreateOrTransferFailure(lisaManager, creationRequest, errorResponse, action)
      } recover { case e: Exception =>
        logger.error(
          s"[AccountController][processAccountCreation]: An error occurred due to ${e.getMessage} returning internal server error for lisaManager : $lisaManager"
        )
  ...
  private def processAccountTransfer(lisaManager: String, transferRequest: CreateLisaAccountTransferRequest)(implicit
    hc: HeaderCarrier,
    startTime: Long
  ) = {
    ...
    hasValidDatesForTransfer(lisaManager, transferRequest) { () =>
      service.transferAccount(lisaManager, transferRequest).map {
        case CreateLisaAccountSuccessResponse(accountId) =>
          ...
          logger.info(
            s"[AccountController][processAccountTransfer]  CreateLisaAccountSuccessResponse accountId : $accountId, lisaManager : $lisaManager"
          )
        ...
        case CreateLisaAccountAlreadyExistsResponse      =>
          logger.info(
            s"[AccountController][processAccountTransfer]  CreateLisaAccountAlreadyExistsResponse lisaManager : $lisaManager"
          )
          handleCreateOrTransferFailure(
            lisaManager,
            transferRequest,
            ErrorAccountAlreadyExists(transferRequest.accountId),
            action
          )
        case error: CreateLisaAccountResponse            =>
          val errorResponse = transferLisaAccountErrorMap.getOrElse(error, ErrorInternalServerError)
          logger.error(
            s"[AccountController][processAccountTransfer]  CreateLisaAccountAlreadyExistsResponse " +
        ...
        logger.error(
          s"[AccountController][processAccountTransfer]: An error occurred in due to ${e.getMessage} returning internal server error for lisaManager : $lisaManager"
        )
  ...
  private def hasValidDatesForCreation(lisaManager: String, creationRequest: CreateLisaAccountCreationRequest)(
    ...
    if (creationRequest.firstSubscriptionDate.isBefore(LISA_START_DATE)) {
      auditService.audit(
        ...
        path = getCreateOrTransferEndpointUrl(lisaManager),
        auditData = creationRequest.toStringMap ++ Map(ZREF -> lisaManager, "reasonNotCreated" -> "FORBIDDEN")
      )
  ...
  private def hasValidDatesForTransfer(lisaManager: String, transferRequest: CreateLisaAccountTransferRequest)(
    ...
    val transferInDateError        = Option(transferRequest.transferAccount.transferInDate.isBefore(LISA_START_DATE))
      .collect { case true =>
        ErrorValidation(
          DATE_ERROR,
          LISA_START_DATE_ERROR.format("transferInDate"),
          Some("/transferAccount/transferInDate")
        )
      }
    ...
    if (errors.nonEmpty) {
      auditService.audit(
        ...
        path = getCreateOrTransferEndpointUrl(lisaManager),
        auditData = transferRequest.toStringMap ++ Map(ZREF -> lisaManager, "reasonNotCreated" -> "FORBIDDEN")
      )
  ...
  private def handleCreateOrTransferFailure(
    lisaManager: String,
    requestData: Product,
    e: ErrorResponse,
    action: String
    ...
    logger.error(s"[AccountController][handleCreateOrTransferFailure] lisaManager : $lisaManager , action : $action")
