...
class BonusPaymentController @Inject() (
    ...
    validateHeader(parse).async { implicit request =>
      withValidLMRN(lisaManager) { () =>
        withValidAccountId(accountId) { () =>
          logger.info(
            s"""[BonusPaymentController][requestBonusPayment] lisaManager: $lisaManager, accountId : $accountId"""
          )
          withApiVersion {
            case Some(VERSION_1) =>
  ...
  def getBonusPayment(lisaManager: String, accountId: String, transactionId: String): Action[AnyContent] =
    validateHeader(parse).async { implicit request =>
      implicit val startTime: Long = System.currentTimeMillis()
      logger.info(
        s"[BonusPaymentController][getBonusPayment] lisaManager: $lisaManager, accountId : $accountId, transactionId : $transactionId"
      )
      withValidLMRN(lisaManager) { () =>
  ...
  private def processGetBonusPayment(lisaManager: String, accountId: String, transactionId: String)(implicit
    ...
    getService.getBonusOrWithdrawal(lisaManager, accountId, transactionId).flatMap {
        ...
        withApiVersion {
          case Some(VERSION_1) =>
            if (response.bonuses.claimReason == "Superseded Bonus") {
              ...
              logger.warn(
                s"[BonusPaymentController][processGetBonusPayment] API v1 received a superseded bonus claim. ID was $transactionId"
              )
              Future.successful(InternalServerError(ErrorInternalServerError.asJson))
            } else {
              logger.info(s"""[BonusPaymentController][processGetBonusPayment] In Version1, lisaManager: $lisaManager,
          ...
          case Some(VERSION_2) =>
            logger.info(s"""[BonusPaymentController][processGetBonusPayment] In Version2, lisaManager: $lisaManager,
                 | accountId : $accountId, transactionId : $transactionId""".stripMargin)
            getBonusPaymentAudit(lisaManager, accountId, transactionId)
      ...
      case GetBonusOrWithdrawalInvestorNotFoundResponse =>
        logger.info(
          s"""[BonusPaymentController][processGetBonusPayment] Received GetBonusOrWithdrawalInvestorNotFoundResponse for lisaManager : $lisaManager | accountId : $accountId, transactionId : $transactionId""".stripMargin
        )
      ...
      case GetBonusOrWithdrawalServiceUnavailableResponse =>
        logger.info(
          s"""[BonusPaymentController][processGetBonusPayment] Received GetBonusOrWithdrawalServiceUnavailableResponse for lisaManager : $lisaManager | accountId : $accountId, transactionId : $transactionId""".stripMargin
        )
  ...
  private def getBonusPaymentAudit(
    ...
    logger.info(s"""[BonusPaymentController][getBonusPaymentAudit] calling audit lisaManager: $lisaManager,
         | accountId : $accountId, transactionId : $transactionId""".stripMargin)
  ...
  private def handleSuccess(
    ...
    logger.info(
      s"[BonusPaymentController][handleSuccess] Matched success response for lisaManager : $lisaManager, account : $accountId"
    )
  ...
  private def handleFailure(
    ...
    logger.info(
      s"[BonusPaymentController][handleFailure] Matched failure response for lisaManager : $lisaManager, account : $accountId"
    )
  ...
  private def handleLifeEventNotProvided(
    ...
    logger.info(
      s"[BonusPaymentController][handleLifeEventNotProvided] Life event not provided for lisaManager : $lisaManager, account : $accountId"
    )
