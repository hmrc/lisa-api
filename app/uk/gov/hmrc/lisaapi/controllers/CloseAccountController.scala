...
class CloseAccountController @Inject() (
  ...
  def closeLisaAccount(lisaManager: String, accountId: String): Action[AnyContent] =
    (validateHeader(parse) andThen validateLMRN(lisaManager) andThen validateAccountId(accountId)).async {
      implicit request =>
        ...
        logger.info(
          s"""[CloseAccountController][closeLisaAccount]  started lisaManager : $lisaManager , accountId : $accountId"""
        )
            ...
            hasValidDatesForClosure(lisaManager, accountId, requestData) { () =>
              service.closeAccount(lisaManager, accountId, requestData).map {
                case CloseLisaAccountSuccessResponse(`accountId`) =>
                  ...
                  logger.info(s"""[CloseAccountController][closeLisaAccount] close lisa account success for
                ...
                logger.error(
                  s"[CloseAccountController][closeLisaAccount]: An error occurred due to ${e.getMessage} returning internal " +
  ...
  private def handleFailure(
    ...
    logger.info(
      s"[CloseAccountController][handleFailure] lisaManager : $lisaManager , accountId : $accountId response : $response"
    )
