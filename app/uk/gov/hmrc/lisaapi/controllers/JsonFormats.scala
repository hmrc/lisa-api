...
trait JsonFormats {
  ...
  implicit val createLisaInvestorRequestReads: Reads[CreateLisaInvestorRequest] = (
    ...
    (JsPath \ "firstName").read(Reads.pattern(nameRegex, "error.formatting.firstName")).map[String](str => str.toUpperCase) and
    (JsPath \ "lastName").read(Reads.pattern(nameRegex, "error.formatting.lastName")).map[String](str => str.toUpperCase) and
