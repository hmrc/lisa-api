  ...
  def reportLisaLifeEvent(lisaManager: String, accountId: String): Action[AnyContent] =
    (validateHeader(parse) andThen validateLMRN(lisaManager) andThen validateAccountId(accountId)).async {
      implicit request =>
        ...
        logger.info(
          s"""[LifeEventController][reportLisaLifeEvent]  accountId : $accountId, lisaManager : $lisaManager"""
        )
          ...
          req =>
            withValidDates(lisaManager, accountId, req) { () =>
              service.reportLifeEvent(lisaManager, accountId, req) flatMap { res =>
                logger.info(
                  s"""[LifeEventController][reportLisaLifeEvent] lisaManager : $lisaManager , response : ${res.toString}"""
                )
                res match {
                  case ReportLifeEventSuccessResponse(lifeEventId) =>
                    logger.info(
                      s"""[LifeEventController][reportLisaLifeEvent] Matched Valid Response lisaManager : $lisaManager"""
                    )
                  ...
                  case res: ReportLifeEventResponse                =>
                    withApiVersion {
                      case Some(VERSION_1) =>
                        ...
                        logger.error(s"""[LifeEventController][reportLisaLifeEvent] ReportLifeEventResponse
                      ...
                      case Some(VERSION_2) =>
                        ...
                        logger.error(
                          s"[LifeEventController][reportLisaLifeEvent] internal server error for" +
