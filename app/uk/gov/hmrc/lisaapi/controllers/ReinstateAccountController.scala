...
class ReinstateAccountController @Inject() (
  ...
  private def processReinstateAccount(lisaManager: String, accountId: String)(implicit
    ...
    service.reinstateAccountService(lisaManager, accountId).map {
      case _: ReinstateLisaAccountSuccessResponse                    =>
        ...
        logger.info(
          s"[ReinstateAccountController][processReinstateAccount] success response for lisaManager : $lisaManager , accountId : $accountId"
        )
      ...
      logger.error(
        s"[ReinstateAccountController][processReinstateAccount] An error occurred returning internal server error"
      )
  ...
  private def processReinstateFailure(
    ...
    logger.info(
      s"[ReinstateAccountController][processReinstateFailure] failed for lisaManager : $lisaManager , " +
