...
object ISO8601Date {
  implicit val format: OFormat[ISO8601Date] = Json.format[ISO8601Date]
}
...
object Constants {
  ...
  val elements: Map[String, List[String]] = Map(
    TRANSACTION.toString     -> List("Bonus"),
  ...
  def apply(key: Constants.EnumKey, code: String): String =
    if (elements(key.toString).contains(code)) code
...
object LisaInvestor {
  implicit val format: OFormat[LisaInvestor] = Json.format[LisaInvestor]
}

case class LisaAccount(
  investorID: ID,
  accountID: AccountId,
...
case class TransferAccount(
  ...
  transferredFromLMRN: ReferenceNumber,
  transferInDate: ISO8601Date
)

case class Bonuses(
  bonusDueForPeriod: Amount,
  totalBonusDueYTD: Amount,
  bonusPaidYTD: Option[Amount],
  claimReason: BonusClaimReason
)
...
case class InboundPayments(
  newSubsForPeriod: Option[Amount],
  newSubsYTD: Amount,
  totalSubsForPeriod: Amount,
  totalSubsYTD: Amount
)
...
object Supersede {
  ...
  )((automaticRecoveryAmount, transactionId, transactionAmount, transactionResult, _) =>
    BonusRecovery(
  ...
  )((transactionId, transactionAmount, transactionResult, _) =>
    AdditionalBonus(
      transactionId,
      transactionAmount,
      transactionResult
