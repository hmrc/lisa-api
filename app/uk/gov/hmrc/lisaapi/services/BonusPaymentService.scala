  ...
  def requestBonusPayment(lisaManager: String, accountId: String, request: RequestBonusPaymentRequest)(implicit
    ...
    desConnector.requestBonusPayment(lisaManager, accountId, request) map {
      case successResponse: DesTransactionResponse       =>
        logger.info(
          "[BonusPaymentService][requestBonusPayment] Matched RequestBonusPaymentSuccessResponse and the message is " + successResponse.message
        )
      ...
      case conflictResponse: DesTransactionExistResponse =>
        logger.info(
          s"[BonusPaymentService][requestBonusPayment] Matched DesTransactionExistResponse and the code is : ${conflictResponse.code} for lisaManager : $lisaManager"
        )
      ...
      case DesUnavailableResponse                        =>
        logger.warn(
          s"[BonusPaymentService][requestBonusPayment] Matched DesUnavailableResponse for lisaManager : $lisaManager"
        )
        RequestBonusPaymentServiceUnavailable
      case failureResponse: DesFailureResponse           =>
        logger.warn(
          s"[BonusPaymentService][requestBonusPayment] Matched DesFailureResponse for lisaManager : $lisaManager and the code is : ${failureResponse.code}"
        )
        desFailures.getOrElse(
          failureResponse.code, {
            logger.error(
              s"[BonusPaymentService][requestBonusPayment] Request bonus payment returned error: ${failureResponse.code} for lisaManager : $lisaManager"
            )
