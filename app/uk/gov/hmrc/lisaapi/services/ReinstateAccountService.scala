  ...
  def reinstateAccountService(lisaManager: String, accountId: AccountId)(implicit
    ...
    desConnector.reinstateAccount(lisaManager, accountId) map {
      case successResponse: DesReinstateAccountSuccessResponse =>
        logger.info(
          s"[ReinstateAccountService][reinstateAccountService] Reinstate account success response for lisaManager : $lisaManager"
        )
        ReinstateLisaAccountSuccessResponse(successResponse.code, successResponse.reason)
      case DesUnavailableResponse                              =>
        logger.warn(
          s"[ReinstateAccountService][reinstateAccountService] Reinstate account returned DES service unavailable for lisaManager : $lisaManager"
        )
      ...
      case failureResponse: DesFailureResponse                 =>
        failureResponse.code match {
          ...
          case _                                    =>
            logger.error(
              s"[ReinstateAccountService][reinstateAccountService] Reinstate account returned error ${failureResponse.code} for lisaManager : $lisaManager"
            )
