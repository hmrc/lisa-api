  ...
  def getTransaction(lisaManager: String, accountId: String, transactionId: String)(implicit
    ...
    desConnector.getBonusOrWithdrawal(lisaManager, accountId, transactionId) flatMap {
      ...
      case DesUnavailableResponse                       =>
        logger.warn(s"ITMP returned with 503 for getTransaction, lisaManager is $lisaManager")
        Future.successful(GetTransactionServiceUnavailableResponse)
      case error: DesFailureResponse                    =>
        logger.error(s"Error from ITMP: ${error.code} for getTransaction, lisaManager is $lisaManager")
        Future.successful(
          error.code match {
            ...
            case _                              =>
              logger.error(s"Get transaction returned error: ${error.code} from ITMP for lisaManager $lisaManager")
  ...
  private def handleITMPResponse(
    ...
    itmpResponse.paymentStatus match {
      case TransactionPaymentStatus.PENDING | TransactionPaymentStatus.DUE | TransactionPaymentStatus.VOID |
        ...
        logger.info(s"Received transaction payment status as cancelled from ITMP for lisaManager : $lisaManager")
        Future.successful(
      ...
      case TransactionPaymentStatus.SUPERSEDED =>
        logger.info(s"Received transaction payment status as superseded from ITMP for lisaManager : $lisaManager")
        Future.successful(
          GetTransactionSuccessResponse(
            transactionId = transactionId,
            paymentStatus = itmpResponse.paymentStatus,
      ...
      case TransactionPaymentStatus.PAID       =>
        logger.info(s"Received transaction payment status as paid from ITMP for lisaManager : $lisaManager")
      ...
      case TransactionPaymentStatus.COLLECTED  =>
        logger.info(s"Received transaction payment status as collected from ITMP for lisaManager : $lisaManager")
        handleCollectedTransaction(lisaManager, accountId, transactionId)
      case _                                   =>
        logger.warn(s"Unexpected status: ${itmpResponse.paymentStatus}, returning an error from ITMP for lisaManager : $lisaManager")
    ...
    desConnector.getTransaction(lisaManager, accountId, transactionId) map {
      ...
      case error: DesFailureResponse        =>
        error.code match {
          ...
          case _           =>
            logger.warn(s"Get collected transaction returned error: ${error.code} from ETMP for lisaManager : $lisaManager")
            GetTransactionErrorResponse
        }
    }
    ...
    desConnector.getTransaction(lisaManager, accountId, transactionId) map {
      ...
      case error: DesFailureResponse         =>
        error.code match {
          ...
          case _                   =>
            logger.warn(s"Get paid transaction returned error: ${error.code} from ETMP for lisaManager : $lisaManager")
            GetTransactionErrorResponse
        }
    }
}
