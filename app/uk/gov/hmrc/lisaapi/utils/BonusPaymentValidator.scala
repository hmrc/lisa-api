...
class BonusPaymentValidator @Inject() (currentDateService: CurrentDateService) extends LisaConstants {
  ...
  private val newSubsOrHtbTransferGtZero: BonusPaymentValidationRequest => BonusPaymentValidationRequest =
    (req: BonusPaymentValidationRequest) =>
      if (isBonusRecovery(req.data)) {
        ...
        val subsGtZero   =
          req.data.inboundPayments.newSubsForPeriod.isDefined && req.data.inboundPayments.newSubsForPeriod.get > 0
  ...
  private val newSubsYTDGtZeroIfNewSubsForPeriodGtZero
    : PartialFunction[BonusPaymentValidationRequest, BonusPaymentValidationRequest] = {
    case req: BonusPaymentValidationRequest
        if req.data.inboundPayments.newSubsForPeriod.isDefined &&
          req.data.inboundPayments.newSubsForPeriod.get > 0 &&
          req.data.inboundPayments.newSubsYTD <= 0 =>
      req.copy(errors =
        req.errors :+ ErrorValidation(
          MONETARY_ERROR,
          "newSubsYTD must be more than 0",
          Some(s"$inboundPayments/newSubsYTD")
        )
      )
    case req: BonusPaymentValidationRequest => req
  }

  private val htbTransferTotalYTDGtZeroIfHtbTransferInForPeriodGtZero
    : PartialFunction[BonusPaymentValidationRequest, BonusPaymentValidationRequest] = {
    case req: BonusPaymentValidationRequest
        if req.data.htbTransfer.isDefined &&
      ...
      req.copy(errors =
        req.errors :+ ErrorValidation(
          ...
          Some(s"$htbTransfer/htbTransferTotalYTD")
        )
      )
    case req: BonusPaymentValidationRequest => req
  }

  private val totalSubsForPeriodGtZero
    : PartialFunction[BonusPaymentValidationRequest, BonusPaymentValidationRequest] = {
    case req: BonusPaymentValidationRequest
        if req.data.inboundPayments.totalSubsForPeriod <= 0 && !isBonusRecovery(req.data) =>
      req.copy(errors =
        req.errors :+ ErrorValidation(
          MONETARY_ERROR,
          "totalSubsForPeriod must be more than 0",
          Some(s"$inboundPayments/totalSubsForPeriod")
        )
      )
    case req: BonusPaymentValidationRequest => req
  }

  private val totalSubsYTDGteTotalSubsForPeriod
    : PartialFunction[BonusPaymentValidationRequest, BonusPaymentValidationRequest] = {
    case req: BonusPaymentValidationRequest
        if req.data.inboundPayments.totalSubsYTD < req.data.inboundPayments.totalSubsForPeriod =>
      req.copy(errors =
        req.errors :+
          ErrorValidation(
            MONETARY_ERROR,
            "totalSubsYTD must be more than or equal to totalSubsForPeriod",
            Some(s"$inboundPayments/totalSubsYTD")
          )
      )
    case req: BonusPaymentValidationRequest => req
  }
  ...
  private val bonusDueForPeriodGtZero: PartialFunction[BonusPaymentValidationRequest, BonusPaymentValidationRequest] = {
    case req: BonusPaymentValidationRequest if req.data.bonuses.bonusDueForPeriod <= 0 && !isBonusRecovery(req.data) =>
      req.copy(errors =
        req.errors :+ ErrorValidation(
          MONETARY_ERROR,
          "bonusDueForPeriod must be more than 0",
          Some(s"$bonuses/bonusDueForPeriod")
        )
      )
    case req: BonusPaymentValidationRequest                                                                          => req
  }
  ...
  private val totalBonusDueYTDGtZero: PartialFunction[BonusPaymentValidationRequest, BonusPaymentValidationRequest] = {
    case req: BonusPaymentValidationRequest if req.data.bonuses.totalBonusDueYTD <= 0 && !isBonusRecovery(req.data) =>
      req.copy(errors =
        req.errors :+ ErrorValidation(
          MONETARY_ERROR,
          "totalBonusDueYTD must be more than 0",
          Some(s"$bonuses/totalBonusDueYTD")
        )
      )
    case req: BonusPaymentValidationRequest                                                                         => req
  }
  ...
  private val periodStartDateIsNotInFuture
    ...
    case req: BonusPaymentValidationRequest if req.data.periodStartDate.toDate.after(currentDateService.now().toDate) =>
      req.copy(errors =
        req.errors :+ ErrorValidation(
          DATE_ERROR,
          "The periodStartDate may not be a future date",
          Some(s"/periodStartDate")
        )
      )
    case req: BonusPaymentValidationRequest                                                                           => req
  ...
  private def getErrors(subsExists: Boolean, htbExists: Boolean, eitherGtZero: Boolean): Seq[ErrorValidation] = {
    ...
    if (showSubError)
      newErrs += ErrorValidation(MONETARY_ERROR, errorMessage, Some(s"$inboundPayments/newSubsForPeriod"))
    if (showHtbError)
      newErrs += ErrorValidation(MONETARY_ERROR, errorMessage, Some(s"$htbTransfer/htbTransferInForPeriod"))
