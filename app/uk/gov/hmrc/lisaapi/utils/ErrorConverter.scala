...
trait ErrorConverter {
  ...
  def convert(error: collection.Seq[(JsPath, collection.Seq[JsonValidationError])]): List[ErrorValidation] =
    error.map { e =>
  ...
  private def getErrorDetails(key: String): (String, String) =
    key match {
      ...
      case f2: String if f2.matches("error\\.formatting\\.currencyNegativeDisallowed.*") =>
        ("INVALID_MONETARY_AMOUNT", "Amount cannot be negative, and can only have up to 2 decimal places")
      case f2: String if f2.matches("error\\.formatting\\.currencyNegativeAllowed.*")    =>
        ("INVALID_MONETARY_AMOUNT", "Amount can only have up to 2 decimal places")
      case f3: String if f3.matches("error\\.formatting\\.annualFigures.*")              =>
        ("INVALID_MONETARY_AMOUNT", "Amount cannot be negative")
      ...
      case f6: String if f6.matches("tooLongNameOrNumber")                               =>
        ("INVALID_NAME_OR_NUMBER", "nameOrNumber must be 35 characters or less")
      case f7: String if f7.matches("invalidNameOrNumber")                               =>
        (
      ...
      case f9: String if f9.matches("invalidPostalCode")                                 =>
        ("INVALID_POSTAL_CODE", "Postcode must only include letters a to z and numbers 0 to 9, like AA1 1AA")
