...
class WithdrawalChargeValidator @Inject()(currentDateService: CurrentDateService) extends LisaConstants {
  ...
  private val periodStartDateIsSixth: PartialFunction[WithdrawalChargeValidationRequest, WithdrawalChargeValidationRequest] = {
    case req: WithdrawalChargeValidationRequest if req.data.claimPeriodStartDate.getDayOfMonth() != 6 => {
      req.copy(errors = req.errors :+ ErrorValidation(DATE_ERROR, "The claimPeriodStartDate must be the 6th day of the month", Some(s"/claimPeriodStartDate")))
    }
    case req: WithdrawalChargeValidationRequest => req
  }

  private val periodStartDateIsNotInFuture: PartialFunction[WithdrawalChargeValidationRequest, WithdrawalChargeValidationRequest] = {
    case req: WithdrawalChargeValidationRequest if req.data.claimPeriodStartDate.toDate.after(currentDateService.now().toDate) => {
      req.copy(errors = req.errors :+ ErrorValidation(DATE_ERROR, "The claimPeriodStartDate may not be a future date", Some(s"/claimPeriodStartDate")))
    }
    case req: WithdrawalChargeValidationRequest => req
  }

  private val periodEndDateIsFifthOfMonthAfterPeriodStartDate: (WithdrawalChargeValidationRequest) => WithdrawalChargeValidationRequest =
  ...
  private val periodStartDateIsNotBeforeFirstValidDate: (WithdrawalChargeValidationRequest) => WithdrawalChargeValidationRequest =
    (req: WithdrawalChargeValidationRequest) => {
