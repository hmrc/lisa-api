...
class DesConnectorSpec extends DesConnectorTestHelper {
  ...
  "Create Lisa Investor endpoint" must {
    ...
    "return a populated CreateLisaInvestorSuccessResponse when The DES response has a json body that is in the correct format" in {
      ...
      val response = await(desConnector.createInvestor("Z019283", createInvestorPayload))

      response must be(CreateLisaInvestorSuccessResponse("1234567890"))
      verifyDesPost(createInvestorUrl, Json.toJson(createInvestorPayload))
    }
  ...
  "Create Account endpoint" must {
    ...
    "return a populated success response when DES returns 201 created" in {
      ...
      val response = await(desConnector.createAccount("Z019283", createAccountPayload))

      response mustBe DesAccountResponse("9876543210")
      verifyDesPost(createAccountUrl, Json.toJson(createAccountPayload))
  ...
  "Transfer Account endpoint" must {
    ...
    "return a populated success response when DES returns 201 created" in {
      ...
      val response = await(desConnector.transferAccount("Z019283", transferAccountPayload))

      response mustBe DesAccountResponse("9876543210")
      verifyDesPost(transferAccountUrl, Json.toJson(transferAccountPayload))
    }
  ...
  "Close Lisa Account endpoint" must {
    ...
    "return a DesEmptySuccessResponse when DES returns 200 ok" in {
      ...
      val response = await(desConnector.closeAccount("Z123456", "ABC12345", closeAccountPayload))

      response mustBe DesEmptySuccessResponse
      verifyDesPost(closeAccountUrl, Json.toJson(closeAccountPayload))
    }
  ...
  "Reinstate Lisa Account endpoint" must {
    ...
    "return a populated success response when DES returns 200 ok" in {
      ...
      val response = await(desConnector.reinstateAccount("Z123456", "ABC12345"))

      response mustBe DesReinstateAccountSuccessResponse("SUCCESS", "Account successfully reinstated")
      verifyDesPut(reinstateAccountUrl, Json.toJson(""), withOriginator = true)
    }
  ...
  "Update First Subscription date endpoint" must {
    "return a populated DesUpdateSubscriptionSuccessResponse when the DES response has a json body that is in the correct format" in {
      ...
      val response = await(desConnector.updateFirstSubDate("Z019283", "123456789", updateFirstSubscriptionDatePayload))
      ...
      verifyDesPut(updateFirstSubUrl, Json.toJson(updateFirstSubscriptionDatePayload))
    }
    ...
    "return a failure response" when {
      ...
      "the DES response has no json body" in {
        ...
        val response =
          await(desConnector.updateFirstSubDate("Z019283", "123456789", updateFirstSubscriptionDatePayload))
    ...
    "return a DesTransactionExistResponse when a 409 response is returned with json in the correct format" in {
      stubForPut(updateFirstSubUrl, CONFLICT, """{"code": "x", "reason": "xx", "transactionID": "87654321"}""")
      val response = await(desConnector.updateFirstSubDate("Z019283", "123456789", updateFirstSubscriptionDatePayload))
  ...
  "Report Life Event endpoint" must {
    ...
    "return a populated DesSuccessResponse when the DES response has a json body that is in the correct format" in {
      ...
      val response = await(desConnector.reportLifeEvent("Z123456", "ABC12345", reportLifeEventPayload))

      response mustBe DesLifeEventResponse("87654321")
      verifyDesPost(reportLifeEventUrl, Json.toJson(reportLifeEventPayload)(ReportLifeEventRequest.desWrites))
    }
  ...
  "Retrieve Life Event endpoint" must {
    ...
    "return a Left of DesUnavailableResponse when a 503 is returned" in {
      ...
      val response = await(desConnector.getLifeEvent("Z123456", "ABC12345", "1234567890"))

      response mustBe Left(DesUnavailableResponse)
      verifyDesGet(getLifeEventUrl, withOriginator = true)
    }

    "return a Left of DesFailureResponse" when {

      "a specific failure is returned" in {
        stubForGet(getLifeEventUrl, CONFLICT, """{ "code": "ERROR_CODE", "reason" : "ERROR MESSAGE" }""")
        val response = await(desConnector.getLifeEvent("Z123456", "ABC12345", "1234567890"))
  ...
  "Request Bonus Payment endpoint" must {
    ...
    "return a populated DesTransactionResponse when the DES response has a json body that is in the correct format" in {
      ...
      val response = await(desConnector.requestBonusPayment("Z123456", "ABC12345", requestBonusPaymentPayload))

      response mustBe DesTransactionResponse("87654321", Some("On Time"))
      verifyDesPost(requestBonusUrl, Json.toJson(requestBonusPaymentPayload))
  ...
  "Retrieve Bonus Payment endpoint" must {
    ...
    "return a DesUnavailableResponse when a 503 is returned" in {
      ...
      val response = await(desConnector.getBonusOrWithdrawal("Z123456", "ABC12345", "123456"))

      response mustBe DesUnavailableResponse
      verifyDesGet(getBonusOrWithdrawal, withOriginator = true)
    }
  ...
  "Retrieve Bulk Payment endpoint" must {
    ...
    "return an unavailable response when a 503 is returned" in {
      ...
      val response =
        await(desConnector.getBulkPayment("Z123456", LocalDate.parse("2018-01-01"), LocalDate.parse("2018-01-01")))

      response mustBe DesUnavailableResponse
      verifyDesGet(bulkPaymentUrl, withOriginator = false)
    }

    "return a failure response" when {

      "the DES response is a failure response" in {
        stubForGet(bulkPaymentUrl, OK, """{ "code": "ERROR_CODE", "reason" : "ERROR MESSAGE" }""")
    ...
    "return a success response when the DES response has the expected json response" in {
      val responseString =
        """{
          ...
          |    { "clearedAmount": -1000, "items": [{ "clearingSAPDocument": "ABC123456789", "clearingDate": "2017-06-01" }] },
          |    { "outstandingAmount": -1500.55, "items": [{ "dueDate": "2017-07-01" }] }
          |  ]
          |}""".stripMargin
    ...
    "return a not found response" when {
      ...
      "the DES response has no financial transactions field" in {
        stubForGet(bulkPaymentUrl, OK, """{ "processingDate": "2017-03-07T09:30:00.000Z", "idNumber": "Z1234" }""")
      ...
      "the DES response has no id number field" in {
        ...
        stubForGet(
          bulkPaymentUrl,
          OK,
          """{ "processingDate": "2017-03-07T09:30:00.000Z", "financialTransactions": [] }"""
        )
      ...
      "the DES response has no id number or financial transactions field" in {
        stubForGet(bulkPaymentUrl, OK, """{ "processingDate": "2017-03-07T09:30:00.000Z" }""")
        val response =
          await(desConnector.getBulkPayment("Z123456", LocalDate.parse("2018-01-01"), LocalDate.parse("2018-01-01")))
  ...
  "Retrieve Account endpoint" must {
    ...
    "return a unavailable response when a 503 is returned" in {
      ...
      val response = await(desConnector.getAccountInformation("Z123456", "123456"))

      response mustBe DesUnavailableResponse
      verifyDesGet(getAccountUrl, withOriginator = true)
    }

    "return a failure response" when {

      "the DES response is a failure response" in {
        stubForGet(getAccountUrl, OK, """{ "code": "ERROR_CODE", "reason" : "ERROR MESSAGE" }""")
        val response = await(desConnector.getAccountInformation("Z123456", "123456"))
  ...
  "Report withdrawal endpoint" must {
    ...
    implicit val writes: Writes[SupersededWithdrawalChargeRequest] =
    ...
    "return a populated DesTransactionResponse when the DES response has a json body that is in the correct format" in {
      ...
      val response = await(desConnector.reportWithdrawalCharge("Z123456", "ABC12345", reportWithdrawalPayload))

      response mustBe DesTransactionResponse("87654321", Some("On Time"))
      verifyDesPost(withdrawalUrl, Json.toJson(reportWithdrawalPayload))
    }
