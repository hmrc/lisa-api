...
class CloseAccountControllerSpec extends ControllerTestFixture {
  ...
  val closeAccountController: CloseAccountController = new CloseAccountController(
    mockAuthConnector,
    mockAppContext,
    mockAuditService,
    mockAccountService,
  ...
  def doCloseRequest(jsonString: String, lmrn: String = lisaManager, accId: String = accountId)(
    callback: Future[Result] => Unit
  ): Unit = {
    val res = closeAccountController
      .closeLisaAccount(lmrn, accId)
      .apply(FakeRequest(Helpers.PUT, "/").withHeaders(acceptHeader).withBody(AnyContentAsJson(Json.parse(jsonString))))

    callback(res)
  }

  def doSyncCloseRequest(jsonString: String)(callback: Result => Unit): Unit = {
    val res = await(
      closeAccountController
        .closeLisaAccount(lisaManager, accountId)
        .apply(
