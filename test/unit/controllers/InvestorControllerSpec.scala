...
class InvestorControllerSpec extends ControllerTestFixture {
  ...
  val investorController: InvestorController = new InvestorController(
    mockAuthConnector,
    mockAppContext,
    mockInvestorService,
    mockAuditService,
  ...
  "The Investor Controller" should {
    ...
    "return with status 406 not acceptable" when {
      "given an invalid accept header" in {
        ...
        val res = investorController
          .createLisaInvestor(lisaManager)
          .apply(FakeRequest(Helpers.PUT, "/").withHeaders(("accept", "application/vnd.hmrcc.1.0+json")))
    ...
    "return with status 500 internal server error" when {
      "an exception gets thrown" in {
        when(mockInvestorService.createInvestor(any(), any())(any())).thenThrow(new RuntimeException("Test"))
    ...
    "convert names to uppercase" when {
      "given standard a-z characters" in {
        ...
        val res = investorController
          .createLisaInvestor(lisaManager)
          .apply(FakeRequest(Helpers.PUT, "/").withHeaders(acceptHeader).withBody(AnyContentAsJson(Json.parse(json))))

        await(res)

        verify(mockInvestorService).createInvestor(
          matchersEquals(lisaManager),
          matchersEquals(
            CreateLisaInvestorRequest(
              investorNINO = "AB123456D",
              firstName = "RICK",
              lastName = "SANCHEZ",
