...
class ReinstateAccountControllerSpec extends ControllerTestFixture {
  ...
  val reinstateAccountController: ReinstateAccountController = new ReinstateAccountController(
    mockAuthConnector,
    mockAppContext,
  ...
  def doReinstateRequest(jsonString: String, lmrn: String = lisaManager)(callback: Future[Result] => Unit): Unit = {
    val res = reinstateAccountController
      .reinstateAccount(lmrn)
      .apply(FakeRequest(Helpers.PUT, "/").withHeaders(acceptHeader).withBody(AnyContentAsJson(Json.parse(jsonString))))

    callback(res)
  }

  def doSyncReinstateAccount(jsonString: String)(callback: Result => Unit): Unit = {
    val res = await(
      reinstateAccountController
        .reinstateAccount(lisaManager)
        .apply(
